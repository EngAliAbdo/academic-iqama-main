...
async function ensureIdentifierAvailable(identifier: string, email: string) {
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    await ensureIdentifierAvailable(identifier, email);
    ...
    if (createUserError || !createdUser.user) {
      return jsonResponse({
        error: createUserError?.message ?? "تعذر إنشاء المستخدم في نظام المصادقة.",
      }, 400);
    }
    ...
    if (profileError || !profile) {
      return jsonResponse({
        error: profileError?.message ?? "تم إنشاء المستخدم ولكن تعذر تحديث ملفه الشخصي.",
      }, 500);
    }
